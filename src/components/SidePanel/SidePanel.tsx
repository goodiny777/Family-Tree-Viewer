...
function PanelContent() {
  ...
  const { sidePanelTab, setSidePanelTab } = useStore((state) => state.ui)

  return (
    <Tabs
      value={sidePanelTab}
      onChange={(value) => setSidePanelTab(value as 'info' | 'settings')}
      className="p-3 flex-1 flex flex-col min-h-0"
    >
      <TabsList>
...
export function SidePanel() {
  const { t } = useTranslation()
  const { isSidePanelOpen, sidePanelTab, setSidePanelOpen } = useStore((state) => state.ui)
  ...
  if (isMobile) {
    const title = sidePanelTab === 'info' ? t('sidePanel.personInfo') : t('sidePanel.settings')

    return (
      <BottomSheet isOpen={isSidePanelOpen} onClose={handleClose} title={title}>
        <PanelContent />
      </BottomSheet>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        ...
        {/* Content with top padding for close button */}
        <div className="pt-12 flex-1 flex flex-col min-h-0">
          <PanelContent />
        </div>
      </div>
    </aside>
